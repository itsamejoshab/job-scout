...
// WakeProcessPending signals the pending dispatcher singleton, starting it
// first when it is not running. The signal carries no payload because the
// dispatcher always reloads pending rows from the database. A dispatcher that
// is already running is the wanted state, not an error.
