...
`make up` starts Postgres, Temporal, the API, and the worker. On boot, the API creates or updates two Temporal schedules: `jobscout-scrape` and `jobscout-notify`. The default scrape interval is 10 minutes. The default notify cron is 07:39, 17:39, and 20:39 in `America/New_York`.
